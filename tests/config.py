...
class TestConfig:
    ...
    def to_fs_properties(self) -> Dict[str, str]:
        ...
        if backend == "local":
            ...
            props = {}
            if self.root_path:
                props["root_path"] = self.root_path
            return props
        ...
        if "cloud_provider" in config:
            props["cloud_provider"] = config["cloud_provider"]
        if "address" in config:
            props["address"] = config["address"]
        if self.bucket_name:
            props["bucket_name"] = self.bucket_name
        if "access_key" in config:
            props["access_key"] = config["access_key"]
        if "secret_key" in config:
            props["secret_key"] = config["secret_key"]
        if "region" in config:
    ...
    def get_writer_properties(
        ...
        **extra_props
        ...
        if file_rolling_size is not None:
            props["writer.file_rolling.size"] = str(file_rolling_size)
        if buffer_size is not None:
            props["writer.buffer_size"] = str(buffer_size)
        if compression is not None:
            props["writer.compression"] = compression
    ...
    def get_transaction_properties(
        ...
        **extra_props
        ...
        """
        props = self.to_fs_properties()
        props["transaction.commit.num-retries"] = str(num_retries)
        props.update({k: str(v) for k, v in extra_props.items()})
        return props
