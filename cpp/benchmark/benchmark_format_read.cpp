...
class FormatReadBenchmark : public FormatBenchFixture {
  public:
  ...
  protected:
  // Prepare test data by writing to storage
  arrow::Status PrepareTestData(const std::string& format,
                                const DataSizeConfig& data_config,
                                std::shared_ptr<ColumnGroups>& out_cgs,
                                std::string& out_path) {
    out_path = GetUniquePath(format + "_read_test");

    ARROW_ASSIGN_OR_RAISE(auto batch, CreateTestData(schema_, 0, true, data_config.num_rows, data_config.vector_dim,
                                                     data_config.string_length));
  ...
  // Get projection columns based on count
  std::shared_ptr<std::vector<std::string>> GetProjection(size_t num_columns) {
    std::vector<std::string> all_columns = {"id", "name", "value", "vector"};
    auto projection = std::make_shared<std::vector<std::string>>();
    for (size_t i = 0; i < std::min(num_columns, all_columns.size()); ++i) {
      projection->push_back(all_columns[i]);
    }
    return projection;
  }
...
BENCHMARK_DEFINE_F(FormatReadBenchmark, ReadFullScan)(::benchmark::State& st) {
  ...
  std::string path;
  BENCH_ASSERT_STATUS_OK(PrepareTestData(format, data_config, cgs, path), st);
  ...
  st.SetLabel(format + "/" + std::to_string(num_threads) + "T/" + memory_config.name);
}
...
BENCHMARK_DEFINE_F(FormatReadBenchmark, ReadTake)(::benchmark::State& st) {
  ...
  MemoryConfig memory_config = MemoryConfig::FromIndex(memory_config_idx);
  DataSizeConfig data_config = DataSizeConfig::Large();  // Use large size for take tests
  ...
  auto indices = GenerateIndices(dist, take_count, static_cast<int64_t>(data_config.num_rows));
  ...
  st.SetLabel(format + "/" + std::to_string(take_count) + "rows/" + IndexDistributionName(dist) + "/" +
              std::to_string(num_threads) + "T/" + memory_config.name);
}
