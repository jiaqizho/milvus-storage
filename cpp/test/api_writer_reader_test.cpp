...
TEST_P(APIWriterReaderTest, TestGetChunksSliced) {
  ...
  // all
  {
    ASSERT_AND_ASSIGN(auto chunks,
                      chunk_reader->get_chunks(chunk_indices, 0 /* parallelism */, true /* need_sliced */));
  ...
  // random test
  {
    ...
    for (size_t i = 1; i <= random_times; ++i) {
      ...
      ASSERT_AND_ASSIGN(auto chunks,
                        chunk_reader->get_chunks(chunkidx_samples, 0 /* parallelism */, true /* need_sliced */));
