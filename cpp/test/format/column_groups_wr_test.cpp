...
TEST_P(ColumnGroupsWRTest, TestTakeAsync) {
  ...
  for (const auto& row_indices : test_row_indices) {
    ...
    for (const auto& task : tasks) {
      auto future = take_reader->take_async(task, AsyncReadOptions{});
...
TEST_P(ColumnGroupsWRTest, TestGetChunksAsync) {
  ...
  for (const auto& task : tasks) {
    auto future = chunk_reader->get_chunks_async(task, AsyncReadOptions{});
  ...
  ASSERT_AND_ASSIGN(auto public_chunk_reader_unique, reader->get_chunk_reader(0));
  std::shared_ptr<ChunkReader> public_chunk_reader(std::move(public_chunk_reader_unique));

  std::vector<int64_t> post_handler_chunks = {0};
  if (total_chunks > 1) {
    post_handler_chunks.push_back(1);
  }
  AsyncReadOptions options{
      .read_parallelism = 2,
      .materialize_executor = arrow::internal::GetCpuThreadPool(),
  };
  auto post_future = get_chunks_async_then(
      public_chunk_reader, post_handler_chunks, options,
      [executor = options.materialize_executor](RecordBatchVector&& batches) -> arrow::Result<bool> {
        return executor->OwnsThisThread() && !batches.empty();
      });
  ASSERT_AND_ASSIGN(auto ran_on_executor, std::move(post_future).get());
  ASSERT_TRUE(ran_on_executor);
}
