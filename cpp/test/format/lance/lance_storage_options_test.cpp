...
TEST_F(LanceStorageOptionsTest, AwsKeys) {
  auto opts = ToWriterOptions(MakeAwsConfig());
  ...
  EXPECT_EQ(opts.count("milvus_lance_io_parallelism"), 0);
}
...
TEST_F(LanceStorageOptionsTest, AzureKeys) {
  ...
  auto opts = ToWriterOptions(config);
...
TEST_F(LanceStorageOptionsTest, WriterRejectsAzureCredentialBroker) {
  ...
  EXPECT_THROW(ToWriterOptions(config), std::runtime_error);
}
...
TEST_F(LanceStorageOptionsTest, ReadOptionsContainNoCredentials) {
  ...
  EXPECT_EQ(options["milvus_lance_io_parallelism"], "64");
