...
static inline expr::Expr build_v2_projection(const std::vector<std::string>& ncs) {
  return expr::select(std::vector<std::string_view>(ncs.begin(), ncs.end()), expr::root());
}

static void remove_v2_metadata_from_schema(ArrowSchema* schema) {
  assert(schema != nullptr);
  for (int64_t i = 0; i < schema->n_children; ++i) {
    remove_v2_metadata_from_schema(schema->children[i]);
  }
  schema->metadata = nullptr;
}

static inline arrow::Result<ArrowSchema> export_v2_c_arrow_schema(std::shared_ptr<arrow::Schema> schema) {
  ArrowSchema c_arrow_schema;

  ARROW_RETURN_NOT_OK(arrow::ExportSchema(*schema, &c_arrow_schema));
  remove_v2_metadata_from_schema(&c_arrow_schema);

  return std::move(c_arrow_schema);
}

static std::vector<RowGroupInfo> create_v2_row_group_infos(uint64_t memory_usage_in_file,
...
static std::vector<uint64_t> recalc_v2_row_ranges(const std::vector<uint64_t>& original_ranges,
...
VortexV2FormatReader::VortexV2FormatReader(const std::shared_ptr<arrow::fs::FileSystem>& fs,
...
arrow::Status VortexV2FormatReader::open() {
  assert(!vxfile_);
  ...
  row_group_infos_ =
      create_v2_row_group_infos(total_mem_usage(), rows(), recalc_v2_row_ranges(row_ranges(), logical_chunk_rows_));

  return arrow::Status::OK();
}

arrow::Result<std::vector<RowGroupInfo>> VortexV2FormatReader::get_row_group_infos() {
  assert(vxfile_);
  return row_group_infos_;
}

arrow::Result<std::shared_ptr<arrow::RecordBatch>> VortexV2FormatReader::get_chunk(const int& row_group_index) {
  assert(vxfile_);
...
arrow::Result<std::vector<std::shared_ptr<arrow::RecordBatch>>> VortexV2FormatReader::get_chunks(
...
arrow::Result<std::shared_ptr<FormatReader>> VortexV2FormatReader::clone_reader() {
  assert(vxfile_);  // already opened
  return this->shared_from_this();
}

arrow::Result<ArrowArrayStream> VortexV2FormatReader::read(uint64_t row_start, uint64_t row_end) {
  auto scan_builder = vxfile_->CreateScanBuilder();
  if (!proj_cols_.empty()) {
    scan_builder.WithProjection(build_v2_projection(proj_cols_));
  }

  if (schema_) {
    ARROW_ASSIGN_OR_RAISE(auto c_arrow_schema, export_v2_c_arrow_schema(schema_));
...
arrow::Result<std::shared_ptr<arrow::RecordBatchReader>> VortexV2FormatReader::streaming_read(uint64_t row_start,
                                                                                              uint64_t row_end) {
  ARROW_ASSIGN_OR_RAISE(auto array_stream, read(row_start, row_end));
  return arrow::ImportRecordBatchReader(&array_stream);
}

arrow::Result<std::shared_ptr<arrow::ChunkedArray>> VortexV2FormatReader::blocking_read(uint64_t row_start,
                                                                                        uint64_t row_end) {
  ARROW_ASSIGN_OR_RAISE(auto array_stream, read(row_start, row_end));
  return arrow::ImportChunkedArray(&array_stream);
}

arrow::Result<std::shared_ptr<arrow::Table>> VortexV2FormatReader::take(const std::vector<int64_t>& row_indices) {
  assert(vxfile_);
  auto scan_builder = vxfile_->CreateScanBuilder();
  if (!proj_cols_.empty()) {
    scan_builder.WithProjection(build_v2_projection(proj_cols_));
...
arrow::Result<std::shared_ptr<arrow::RecordBatchReader>> VortexV2FormatReader::read_with_range(
    const uint64_t& start_offset, const uint64_t& end_offset) {
  assert(vxfile_);
  return streaming_read(start_offset, end_offset);
}

uint64_t VortexV2FormatReader::total_mem_usage() {
