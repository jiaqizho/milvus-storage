...
StorageOptions ToWriterOptions(const ArrowFileSystemConfig& config) {
  ...
  if (provider == kCloudProviderAWS) {
    if (!config.role_arn.empty()) {
      throw std::runtime_error("Lance native writer does not support AWS AssumeRole");
    }
    if (!config.use_iam) {
    ...
    if (config.IsAzureCredentialBrokerEnabled()) {
      throw std::runtime_error("Lance native writer does not support Azure credential broker");
    }
    ...
    if (!config.gcp_target_service_account.empty()) {
      throw std::runtime_error("Lance native writer does not support GCP impersonation");
    }
    if (!config.use_iam && (!config.access_key_id.empty() || !config.access_key_value.empty())) {
      throw std::runtime_error(
          "Lance native writer does not support GCP HMAC credentials; use an AWS S3-compatible endpoint");
    }
  } else if (provider == kCloudProviderAliyun) {
    if (!config.role_arn.empty()) {
      throw std::runtime_error("Lance native writer does not support Aliyun role credentials");
    }
    if (!config.use_iam) {
