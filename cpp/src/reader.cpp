...
class ChunkReaderImpl : public ChunkReader {
  ...
  [[nodiscard]] arrow::Result<std::vector<std::shared_ptr<arrow::RecordBatch>>> get_chunks(
      const std::vector<int64_t>& chunk_indices, int64_t parallelism, bool need_sliced) override;
...
arrow::Result<std::vector<std::shared_ptr<arrow::RecordBatch>>> ChunkReaderImpl::get_chunks(
    const std::vector<int64_t>& chunk_indices, int64_t /*parallelism*/, bool need_sliced) {
#if 0
  ...
  // no need to slice
  if (!need_sliced || results.size() == chunk_indices.size()) {
    return results;
  }
  ...
  for (size_t i = 0; i < chunk_indices.size(); ++i) {
    ...
    if (curr_rb_offset + target_number_of_rows > rb->num_rows()) {
      return arrow::Status::Invalid(
    ...
    if (curr_rb_offset + target_number_of_rows == rb->num_rows()) {
      if (curr_rb_offset == 0) {
        ...
        auto sliced_rb = rb->Slice(curr_rb_offset, target_number_of_rows);
        sliced_results.emplace_back(sliced_rb);
      }
      // move to next record batch
      curr_rb_offset = 0;
      curr_rb_index++;
    } else {
      // need to slice
      ...
      // move the offset, but not move to next record batch
